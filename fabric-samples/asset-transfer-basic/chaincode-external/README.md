...
To set the path within the peer container, you will need to modify the docker compose file to mount a couple of additional volumes. Open the file `test-network/compose/docker/docker-compose-test-net.yaml`, and add to the `volumes` section of both `peer0.org1.mine.com` and `peer0.org2.mine.com` the following two lines:
...
```
setGlobals 1
peer lifecycle chaincode queryinstalled --peerAddresses localhost:7051 --tlsRootCertFiles organizations/peerOrganizations/org1.mine.com/peers/peer0.org1.mine.com/tls/ca.crt
```
...
```
docker run -it --rm --name asset-transfer-basic.org1.mine.com --hostname asset-transfer-basic.org1.mine.com --env-file chaincode.env --network=fabric_test hyperledger/asset-transfer-basic
```
...
```
setGlobals 2
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.mine.com --tls --cafile "$PWD/organizations/ordererOrganizations/mine.com/orderers/orderer.mine.com/msp/tlscacerts/tlsca.mine.com-cert.pem" --channelID mychannel --name basic --version 1.0 --package-id $CHAINCODE_ID --sequence 1

setGlobals 1
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.mine.com --tls --cafile "$PWD/organizations/ordererOrganizations/mine.com/orderers/orderer.mine.com/msp/tlscacerts/tlsca.mine.com-cert.pem" --channelID mychannel --name basic --version 1.0 --package-id $CHAINCODE_ID --sequence 1

peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.mine.com --tls --cafile "$PWD/organizations/ordererOrganizations/mine.com/orderers/orderer.mine.com/msp/tlscacerts/tlsca.mine.com-cert.pem" --channelID mychannel --name basic --peerAddresses localhost:7051 --tlsRootCertFiles "$PWD/organizations/peerOrganizations/org1.mine.com/peers/peer0.org1.mine.com/tls/ca.crt" --peerAddresses localhost:9051 --tlsRootCertFiles organizations/peerOrganizations/org2.mine.com/peers/peer0.org2.mine.com/tls/ca.crt --version 1.0 --sequence 1
```
...
**Note:** This section uses an mine of self-signed certificate. You may use your organization hosted CA to issue the certificate and generate a key for production deployment.
...
```
openssl req -nodes -x509 -newkey rsa:4096 -keyout crypto/key1.pem -out crypto/cert1.pem -subj "/C=IN/ST=KA/L=Bangalore/O=mine Inc/OU=Developer/CN=asset-transfer-basic.org1.mine.com/emailAddress=dev@asset-transfer-basic.org1.mine.com"
```

For `org2.mine.com`

```
openssl req -nodes -x509 -newkey rsa:4096 -keyout crypto/key2.pem -out crypto/cert2.pem -subj "/C=IN/ST=KA/L=Bangalore/O=mine Inc/OU=Developer/CN=asset-transfer-basic.org2.mine.com/emailAddress=dev@asset-transfer-basic.org2.mine.com"
```

- Copy the CA file contents for both `org1.mine.com` & `org2.mine.com`

```
cp ../../test-network/organizations/peerOrganizations/org1.mine.com/ca/ca.org1.mine.com-cert.pem crypto/rootcert1.pem
cp ../../test-network/organizations/peerOrganizations/org2.mine.com/ca/ca.org2.mine.com-cert.pem crypto/rootcert2.pem
```
...
```
{
  "address": "asset-transfer-basic.org1.mine.com:9999",
  "dial_timeout": "10s",
  "tls_required": true,
...
- Follow the instructions in [Package](#packaging-and-installing-chaincode) and [Install](#installing-the-external-chaincode) steps for each organization. Remember that the chaincode server's address for the second organization is `asset-transfer-basic.org2.mine.com:9999`.
